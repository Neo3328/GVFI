...
int main() {
  assert(std::strcmp(gvfi_version(), "gvfi_native/0.4.0") == 0);
  ...
  if (info.ncnn_enabled == 0) {
    ...
    assert(gvfi_load_model(handle, "test.param", "test.bin") == GVFI_FAILED);
    assert(gvfi_process(handle, &frame0, &frame1, 0.5, &output) ==
           GVFI_INVALID_ARGUMENT);
  }
  assert(gvfi_process(handle, nullptr, &frame1, 0.5, &output) ==
         GVFI_INVALID_ARGUMENT);
  assert(gvfi_release(handle) == GVFI_SUCCESS);
  assert(gvfi_release(handle) == GVFI_SUCCESS);
  assert(gvfi_release(nullptr) == GVFI_INVALID_ARGUMENT);
