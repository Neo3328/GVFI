    ...
    variants: {
      ...
      tone: {
        ...
        rail: cn(
          ...
          "rounded-none border-y-0 border-l-0 lg:rounded-[var(--radius-lg)] lg:border",
...
export function Sidebar({
  ...
  return (
    ...
    >
      {brand ? (
        <div className="relative z-[1] border-b border-[var(--glass-border)] px-[var(--space-3)] py-[var(--space-4)]">
          {brand}
        </div>
      ) : null}
      <nav
        className={cn(
          "relative z-[1] flex flex-1 flex-col gap-[var(--space-1)] p-[var(--space-2)]",
      ...
      >
        {items.map((item) => {
          ...
          const classNames = cn(
            compact
              ? "glass-option-chip size-11 flex-col gap-1 rounded-[var(--radius-sm)] px-0 text-[10px]"
              : "glass-option flex min-h-9 items-center gap-[var(--space-2)] rounded-[var(--radius-sm)] px-[var(--space-3)] py-[var(--space-2)] text-[13px]",
            "font-semibold",
            glassFocusRing,
            glassMotion,
            "active:scale-[0.98]",
          ...
          return (
            ...
            >
              {Icon ? <Icon className="size-[18px] shrink-0" strokeWidth={2.25} aria-hidden /> : null}
              {!compact ? <span className="truncate font-semibold">{item.label}</span> : null}
            </Link>
          );
        })}
      </nav>
      {footer ? (
        <div
          className={cn(
            "relative z-[1] border-t border-[var(--glass-border)] p-[var(--space-2)]",
